...
class CommandActionServer(Node):
    ...
    def __init__(self):
        ...
        # кземпляр сообщения Twist спользуется для представления скоростей движения и угловых скоростей в трехмерном пространстве
        self.twist = Twist() 
        self.flag = 0
        # используется для хранения информации о текущем положении и ориентации объекта в трехмерном пространстве.
        self.after_pose = Pose() 
        self.before_pose = Pose()
        self.publisher_ = self.create_publisher(Twist, '/turtle1/cmd_vel', 10)
        #с оздаем экземпляр нового сервера действий
        self._action_server = ActionServer(
            self,
            MessageTurtleCommands,
            'execute_turtle_commands',
            self.execute_callback)
        # тот фрагмент кода создает подписку на тему /turtle1/pose, которая предоставляет сообщения типа Pose. Позволяет узлу получать информацию о положении черепахи
